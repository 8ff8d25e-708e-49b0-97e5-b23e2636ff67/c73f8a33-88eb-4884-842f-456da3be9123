...
def summarise_triples_factory(training_factory, testing_factory, validation_factory):
    ...
    df.index.name = f'Hormone and Organ mapping'
...
def training_loop(cortisol_feature_path, training_data, num_epochs=200, embedding_dim=483, random_seed=35, loss=MarginRankingLoss(margin=1.0)):
    model = RotatE(triples_factory=training_data, random_seed=random_seed, embedding_dim=embedding_dim, loss=loss)
    ...
    # We want to use the whole dataset for the final model
    ...
    # optimizer = optim.Adagrad(params=model.get_grad_params(), lr=0.02)
    optimizer = optim.Adagrad(params=model.get_grad_params(), lr=0.01)
    ...
    negative_sampler = BasicNegativeSampler(
        mapped_triples=training_data.mapped_triples,
        num_negs_per_pos=1,
    )
...
if __name__ == "__main__":
    ...
    # This is training with the training set
    training_loop("./embeddings/organ-features.pt", training)

    # This is the final model with the full dataset (for production)
    training_loop("./embeddings/full-training-set.pt", triples_factory)
